...
export default function AutoPhase({data, setData}: {
    ...
    function handlePointerDown(e: React.PointerEvent) {
        ...
        setX1(p.x); setY1(p.y); setX2(p.x); setY2(p.y); setDragging(true)
    }

    function handlePointerMove(e: React.PointerEvent) {
        if (!dragging) return
        const p = getFieldPos(e); setX2(p.x); setY2(p.y)
    }

    function handlePointerUp(e: React.PointerEvent) {
        if (!dragging) return
        const p = getFieldPos(e); setX2(p.x); setY2(p.y); setDragging(false)
    }
    ...
    function commitCurrentShot() {
        ...
        setShotIndex(null); setFuelShotStack([]); setFuelScoredStack([])
    }
    ...
    return (
        <div className="w-screen h-max flex flex-col p-2 select-none gap-4 text-sm">
            ...
            >
                <img src="/seasons/2026/Field.png" className="absolute inset-0 w-full h-full object-contain pointer-events-none" alt="field" />
                ...
                <svg className="absolute inset-0 w-full h-full pointer-events-none">
                    <line x1={`${viewX(x1) * 100}%`} y1={`${viewY(y1) * 100}%`} x2={`${viewX(x2) * 100}%`} y2={`${viewY(y2) * 100}%`} stroke="#B39EB5" strokeWidth="3" markerEnd="url(#arrow-pastel)" />
                    <defs><marker id="arrow-pastel" markerWidth="5" markerHeight="5" viewBox="0 0 10 10" refX="4" refY="5" orient="auto"><path d="M0,0 L10,5 L0,10 Z" fill="#B39EB5"/></marker></defs>
                </svg>
            </div>
            ...
            <div className="text-center text-sm font-bold">Fired: {fuelShot}</div>
            ...
            <div className="text-center text-sm font-bold">Scored: {fuelScored}</div>
            ...
            <div className="grid grid-cols-2 gap-2">
                <button
                    onClick={() => {
                        setShots(arr => {
                            ...
                            setShotIndex(newIndex); setX1(s.x1); setY1(s.y1); setX2(s.x2); setY2(s.y2);
                            setFuelShotStack([s.fuelShot]); setFuelScoredStack([s.fuelScored])
                            return updated
                        })
                    }}
                ...
                <button
                    onClick={() => { setFuelShotStack([]); setFuelScoredStack([]) }}
