...
export default function MatchDataPostPage() {
    ...
    const predError = result.pred_error
                ...
                <div className="flex-[1.5] flex flex-col overflow-auto bg-zinc-50 border-x border-zinc-200">
                    ...
                    {pred.red_score_pred != null && (
                        <Section label="Predicted vs Actual">
                            <div className="px-6 py-3 space-y-2">
                                <PredActualRow label="Total Score" redPred={pred.red_score_pred} redActual={red.score} bluePred={pred.blue_score_pred} blueActual={blue.score} />
                                <PredActualRow label="Fuel" redPred={pred.red_fuel_pred} redActual={red.hub.total.count} bluePred={pred.blue_fuel_pred} blueActual={blue.hub.total.count} />
                                <PredActualRow label="Tower Pts" redPred={pred.red_climb_pred} redActual={red.tower_points} bluePred={pred.blue_climb_pred} blueActual={blue.tower_points} />
                                <PredActualRow label="Auto Fuel" redPred={pred.red_auto_pred} redActual={red.hub.auto.count} bluePred={pred.blue_auto_pred} blueActual={blue.hub.auto.count} />
                                {predError && (
                                    <div className="flex justify-between text-xs text-zinc-400 pt-1 border-t border-zinc-100">
                                        <span>Pred error: <span className={predError.red > 0 ? "text-green-600" : "text-red-500"}>{predError.red > 0 ? "+" : ""}{predError.red}</span></span>
                                        <span>{predError.pred_winner_correct ? "✓ Correct winner" : "✗ Wrong winner"}</span>
                                        <span>Pred error: <span className={predError.blue > 0 ? "text-green-600" : "text-red-500"}>{predError.blue > 0 ? "+" : ""}{predError.blue}</span></span>
                                    </div>
                                )}
                            </div>
                        </Section>
                    ...
                    <Section label="Score Breakdown">
                        <div className="px-6 py-3 space-y-2">
                            ...
                            <ComparisonBar label="Teleop Points" redVal={red.teleop_points} blueVal={blue.teleop_points} />
                    ...
                    <Section label="Fuel by Phase">
                        <div className="px-6 py-3 space-y-2">
                            ...
                            <ComparisonBar label="Transition" redVal={red.hub.transition.count} blueVal={blue.hub.transition.count} />
                            <ComparisonBar label="Shift 1" redVal={red.hub.shift_1.count} blueVal={blue.hub.shift_1.count} />
                            <ComparisonBar label="Shift 2" redVal={red.hub.shift_2.count} blueVal={blue.hub.shift_2.count} />
                            <ComparisonBar label="Shift 3" redVal={red.hub.shift_3.count} blueVal={blue.hub.shift_3.count} />
                            <ComparisonBar label="Shift 4" redVal={red.hub.shift_4.count} blueVal={blue.hub.shift_4.count} />
                            <ComparisonBar label="Endgame" redVal={red.hub.endgame.count} blueVal={blue.hub.endgame.count} />
                            <div className="border-t border-zinc-100 pt-2">
                                <ComparisonBar label="Total Fuel" redVal={red.hub.total.count} blueVal={blue.hub.total.count} />
                            </div>
                        </div>
                    </Section>
                    ...
                    <Section label="Ranking Points">
                        <div className="px-6 py-3 space-y-2">
                            <RPOutcomeRow label="Energized (≥100)" redAchieved={red.energized} blueAchieved={blue.energized} redProb={pred.red_energized_prob} blueProb={pred.blue_energized_prob} />
                            <RPOutcomeRow label="Supercharged (≥360)" redAchieved={red.supercharged} blueAchieved={blue.supercharged} redProb={pred.red_supercharged_prob} blueProb={pred.blue_supercharged_prob} />
                            <RPOutcomeRow label="Traversal (≥50 tower)" redAchieved={red.traversal} blueAchieved={blue.traversal} redProb={pred.red_traversal_prob} blueProb={pred.blue_traversal_prob} />
                    ...
                    {(red.minor_fouls > 0 || red.major_fouls > 0 || blue.minor_fouls > 0 || blue.major_fouls > 0) && (
                        <Section label="Penalties">
                            <div className="px-6 py-3 space-y-1">
                                ...
                                <ComparisonBar label="Foul Points Given" redVal={blue.foul_points} blueVal={red.foul_points} />
                            </div>
                        </Section>
                    )}
                </div>
...
function ScoreHeader({color, score, rp, isWinner}: { color: "red" | "blue"; score: number; rp: number; isWinner: boolean }) {
        ...
        <div className="space-y-0.5">
            ...
            <div className="flex h-2 gap-0.5">
                ...
                <div className="flex-1">
                    <div className="bg-blue-400 h-full rounded-r transition-all" style={{width: `${(b / max) * 100}%`}} />
                </div>
            </div>
        </div>
    )
...
function RPBadge({achieved, prob, color}: { achieved: boolean; prob?: number; color: "red" | "blue" }) {
    ...
    return (
        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${bg} min-w-[64px] text-center`}>
            {achieved ? "✓" : "✗"}
...
function ClimbResultPanel({color, climbs, teamNames}: { color: "red" | "blue"; climbs: any[]; teamNames: Record<number, string> }) {
        ...
        <div>
            ...
            <div className="space-y-1.5">
                {climbs.map((c: any, i: number) => (
                    <div key={i} className="flex items-center justify-between text-xs bg-white rounded px-2 py-1.5 border border-zinc-100">
                        <div>
